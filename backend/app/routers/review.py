...
def _had_blocking_failure(req: AuthRequest) -> bool:
    """True if a coverage gate (ELIG/COV) failed on the engine's original
    pass over this case. Blocking gates are supposed to be absolute, so the
    engine's "leaning" should never read as an approval when one failed --
    even if the case somehow reached PENDING_REVIEW."""
    criteria = (req.criteria or {}).get("criteria", [])
    return any(c.get("blocking") and not c.get("passed") for c in criteria)
...
def decide(
    ...
    # FIX: a blocking coverage-gate failure means the engine was never
    # leaning "APPROVED" in any meaningful sense, regardless of the necessity
    # score. Without this, a reviewer approving a gated case would be logged
    # as "agreed_with_engine": true, which misrepresents the audit trail.
    if _had_blocking_failure(req):
        engine_leaning = "DENIED"
    else:
        engine_leaning = (
            "APPROVED" if (req.necessity_score or 0) >= 0.5 else "DENIED"
        )
...
def resolve_appeal(
    ...
    req = db.get(AuthRequest, appeal.request_id)
    # FIX: this branch used to flip req.status/decision without ever writing
    # a human-authored explanation, so req.criteria["rationale"] (the
    # engine's original, now-stale verdict) stayed the only rationale on
    # record -- and the frontend banner rendered it in the "approve" color
    # once status flipped to APPROVED, producing a green box that said
    # "Denied on coverage grounds...". Writing reviewer_notes here gives the
    # frontend a current, correctly-toned explanation to show instead.
    if body.status == "OVERTURNED":
        req.decision = "APPROVED"
        req.status = "APPROVED"
        req.decision_source = "REVIEWER"
        req.decision_at = datetime.now(timezone.utc)
        req.reviewer_notes = f"Appeal overturned by {user.full_name}: {body.outcome_notes}"
    else:
        req.status = "DENIED"
        req.reviewer_notes = f"Appeal denial upheld by {user.full_name}: {body.outcome_notes}"
