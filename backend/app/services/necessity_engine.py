...
def evaluate(features: dict, policy_fit: float) -> dict:
    ...
    if blockers:
        decision, status = "DENIED", "AUTO_DENIED"
        # FIX: was joining c["label"] (the criterion's positive-framed name,
        # e.g. "Member eligible on date of service"), which made a denial
        # rationale read as if every gate had passed. Use c["observed"]
        # (e.g. "Not eligible") so the sentence states what actually failed.
        rationale = (
            "Denied on coverage grounds: "
            + "; ".join(
                f"{c['label'].lower()} ({c['observed'].lower()})" for c in blockers
            )
            + "."
        )
        confidence = 0.99
